...
func TestTargetScrape(t *testing.T) {
	...
	select {
	case smpl := <-ch:
		expect(smpl, model.Metric{model.MetricNameLabel: "test_metric", "a": "b"}, 0)
	...
	select {
	case smpl := <-ch:
		expect(smpl, model.Metric{model.MetricNameLabel: "test_metric", "a": "c"}, 1)
	case <-time.After(1 * time.Second):
		t.Fatalf("No sample received")
	}
}
...
func TestTargetScrapeErrorResponse(t *testing.T) {
	...
	err := target.Scrape(context.Background(), make(chan *model.Sample))
	if err == nil {
